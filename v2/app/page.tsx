...
import { newSubmissionKey } from "../lib/submission-key";
...
export default function Home() {
  ...
  /**
   * ONE SUBMIT, ONE ORDER — the browser's half. See lib/submission-key.ts.
   *
   * Minted once for this quote build and sent with EVERY attempt, so the
   * server can tell a retry from a second order. Before this, a submit that
   * succeeded with a lost response — flaky wifi, a backgrounded tab — got
   * pressed again and became a second Printavo order with its own live
   * payment link.
   *
   * A ref, not state: it must not re-render anything, and it must not be
   * regenerated by a render. It is replaced only in startNewQuote(), which
   * is the one place a genuinely NEW order begins.
   */
  const submissionKeyRef = useRef<string>(newSubmissionKey());
  ...
  function buildQuotePayload() {
     ...
     */
    const orderEnvelope = {
      // Same key on a retry, so /api/quote can recognise the second attempt
      // as the same intent. See lib/submission-key.ts.
...
This is an estimate, not a final invoice. Gorilla Salem will confirm pricing, timeline, and artwork readiness before production starts.`;
  ...
  function startNewQuote() {
    // A NEW order, so a new key. Reusing it would make the next order look
    // like a retry of the last one and be refused as a duplicate — which is
    // the failure mode that matters most here, since it silently drops an
    // order rather than duplicating one.
